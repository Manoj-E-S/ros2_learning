    ...
    def check_battery_state(self):
        ...
        if self.battery_state_ == "full":
            if time_now - self.last_time_battery_state_changed_ > 4.0:
                ...
                self.call_set_led_server(BatteryNode.battery_led_index, True)
        else:
            if time_now - self.last_time_battery_state_changed_ > 6.0:
                self.battery_state_ = "full"
                self.get_logger().info("Battery is now full again.")
                self.last_time_battery_state_changed_ = time_now
                self.call_set_led_server(BatteryNode.battery_led_index, False)

    def call_set_led_server(self, led_index, state):
        client = self.create_client(TurnOnLed, "set_led")
        while not client.wait_for_service(1.0):
            self.get_logger().warn("Waiting for Led panel to be activated...")
        ...
        future.add_done_callback(partial(self.callback_call_set_led, led_index=led_index, state=state))

    def callback_call_set_led(self, future, led_index, state):
        try:
            response = future.result()
            self.get_logger().info(f"LED[{BatteryNode.battery_led_index}] ON? {response.success}")
